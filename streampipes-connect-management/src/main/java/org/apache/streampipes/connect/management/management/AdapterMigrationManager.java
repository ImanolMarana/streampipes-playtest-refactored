...
public class AdapterMigrationManager extends AbstractMigrationManager implements IMigrationHandler {
  ...
  public void handleMigrations(SpServiceRegistration extensionsServiceConfig,
    ...
    for (var migrationConfig : migrationConfigs) {
      handleMigrationForConfig(extensionsServiceConfig, migrationConfig);
    }
  }

  private void handleMigrationForConfig(SpServiceRegistration extensionsServiceConfig, ModelMigratorConfig migrationConfig) {
    ...
    for (var adapterDescription : adapterDescriptions) {
      handleMigrationForAdapter(extensionsServiceConfig, migrationConfig, adapterDescription);
    }
  }

  private void handleMigrationForAdapter(SpServiceRegistration extensionsServiceConfig, ModelMigratorConfig migrationConfig, SpServiceRegistration adapterDescription) {
    ...
    if (adapterVersion == migrationConfig.fromVersion()) {
      ...
      if (migrationResult.success()) {
        handleSuccessfulMigration(migrationResult);
      } else {
        handleFailedMigration(extensionsServiceConfig, adapterDescription, migrationResult);
      }
    } else {
      LOG.info(
  ...
  private void handleSuccessfulMigration(MigrationResult migrationResult) {
    ...
    LOG.debug(
        ...
        migrationResult.element().getAppId());
  ...
  private void handleFailedMigration(SpServiceRegistration extensionsServiceConfig, SpServiceRegistration adapterDescription, MigrationResult migrationResult) {
