...
public class UnitFactory {
	...
	public Unit getUnit(URI resource) {
        ...
        try {
            ...
            processStatements(statements, unit, multiplier);
    ...
    private void processStatements(Model statements, Unit unit, Multiplier multiplier) {
        for (Statement statement : statements) {
            if (statement.getPredicate().equals(QUDT.SYMBOL)) {
                ...
                handleTypeStatement(statement, unit);
            }
        }
    }

    private void handleTypeStatement(Statement statement, Unit unit) {
        ...
        if (type instanceof org.eclipse.rdf4j.model.URI) {
            ...
            if (!shouldBeIgnored(typeURI)) {
                try {
                    ...
                    throw new IllegalStateException("Invalid type URI: " + typeURI.stringValue(), e);
                }
            }
        }
    }
