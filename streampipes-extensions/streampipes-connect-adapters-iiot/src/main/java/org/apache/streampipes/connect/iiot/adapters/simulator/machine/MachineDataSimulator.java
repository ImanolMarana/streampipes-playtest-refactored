...
public class MachineDataSimulator implements Runnable {
  ...
  public void run() {
    ...
    while (running) {
      ...
      event = processSensorData(timeDeltaMs, startTimeMs);
  ...
  private Map<String, Object> processSensorData(long timeDeltaMs, long startTimeMs) {
    Map<String, Object> event = new HashMap<>();
    switch (this.selectedSimulatorOption) {
      case "flowrate":
        event = handleFlowrateSimulation(timeDeltaMs, startTimeMs);
        break;
      case "pressure":
        event = handlePressureSimulation(timeDeltaMs, startTimeMs);
        break;
      case "waterlevel":
        event = handleWaterlevelSimulation(timeDeltaMs, startTimeMs);
        break;
      default:
        handleDefaultSimulation();
    }
    return event;
  }

  private Map<String, Object> handleFlowrateSimulation(long timeDeltaMs, long startTimeMs) {
    if (timeDeltaMs > 0 && timeDeltaMs <= 30000) {
      return buildFlowrateEvent(0);
    } else if (timeDeltaMs > 30000 && timeDeltaMs <= 60000) {
      return buildFlowrateEvent(1);
    } else {
      // > 60s
  ...
  private Map<String, Object> handlePressureSimulation(long timeDeltaMs, long startTimeMs) {
    if (timeDeltaMs > 0 && timeDeltaMs <= 30000) {
      return buildPressureEvent(0);
    } else if (timeDeltaMs > 30000 && timeDeltaMs <= 60000) {
      return buildPressureEvent(1);
    } else {
      // > 60s
      // reset start time to start over again
      startTimeMs = System.currentTimeMillis();
  ...
  private Map<String, Object> handleWaterlevelSimulation(long timeDeltaMs, long startTimeMs) {
    if (timeDeltaMs > 0 && timeDeltaMs <= 30000) {
      return buildWaterlevelEvent(0);
    } else if (timeDeltaMs > 30000 && timeDeltaMs <= 60000) {
      return buildWaterlevelEvent(1);
    } else {
      // > 60s
      // reset start time to start over again
      startTimeMs = System.currentTimeMillis();
  ...
  private void handleDefaultSimulation() {
    try {
