...
public class IfmAlMqttAdapter implements StreamPipesAdapter {
  ...
  public void onAdapterStarted(IAdapterParameterExtractor extractor,
    ...
    this.mqttConsumer = new MqttConsumer(
        this.mqttConfig,
        (mqttEvent) -> handleMessage(mqttEvent, collector, adapterRuntimeContext, sensor)
    );
  ...
  private void handleMessage(byte[] mqttEvent, IEventCollector collector, 
                                IAdapterRuntimeContext adapterRuntimeContext, SensorVVB001 sensor) {
    try {
      InputStream in = convertByte(mqttEvent);
      parser.parse(in, (event) -> {
        try {
          processPayload(event, collector, adapterRuntimeContext, sensor);
        } catch (ParseException e) {
          handleParseException(adapterRuntimeContext, e);
        }
      });
    } catch (ParseException e) {
  ...
  private void processPayload(Map<String, Object> event, IEventCollector collector, 
                                  IAdapterRuntimeContext adapterRuntimeContext, SensorVVB001 sensor) throws ParseException {
    ...
    for (int i = 0; i < ports.size(); i++) {
      processPortData(payload, i, collector, adapterRuntimeContext, sensor, serialnumber);
    }
  }

  private void processPortData(Map<String, Object> payload, int portIndex, IEventCollector collector, 
                                  IAdapterRuntimeContext adapterRuntimeContext, SensorVVB001 sensor, 
                                  Object serialnumber) {
    String keyPortInformation = KEY_PORT_INFORMATION.formatted(ports.get(portIndex));

    if (payload.containsKey(keyPortInformation)) {
      handlePortDataPresent(payload, portIndex, collector, adapterRuntimeContext, sensor, serialnumber, keyPortInformation);
    } else {
      handlePortDataMissing(adapterRuntimeContext, portIndex);
    }
  }

  private void handlePortDataPresent(Map<String, Object> payload, int portIndex, IEventCollector collector, 
                                      IAdapterRuntimeContext adapterRuntimeContext, SensorVVB001 sensor, 
                                      Object serialnumber, String keyPortInformation) {
    try {
      Map<String, Object> portResult = getMap(payload, keyPortInformation);

      if (portResult.containsKey("data")) {
        String eventData = (String) portResult.get("data");
        var parsedEvent = sensor.parseEvent(eventData);
        parsedEvent.put("timestamp", System.currentTimeMillis() + portIndex);
        parsedEvent.put("port", "port" + ports.get(portIndex));
        parsedEvent.put(SensorVVB001.IO_LINK_MASTER_SN, serialnumber);
        collector.collect(parsedEvent);
      } else {
        handleMissingEventData(adapterRuntimeContext, portIndex);
      }
    } catch (Exception e) {
      handleDataExtractionError(adapterRuntimeContext, portIndex, e);
    }
  }

  private void handlePortDataMissing(IAdapterRuntimeContext adapterRuntimeContext, int portIndex) {
    if (!missingPortInformationDetected) {
      adapterRuntimeContext
          .getLogger()
          .warn("Event does not contain information about port " + portIndex, "");
      LOG.error("IoLink event does not look like expected. No port information found for port {}.", portIndex);
      missingPortInformationDetected = true;
    }
  }

  private void handleMissingEventData(IAdapterRuntimeContext adapterRuntimeContext, int portIndex) {
    if (!missingEventDataDetected) {
      adapterRuntimeContext
          .getLogger()
          .warn("Payload for port %s does not contain event data".formatted(portIndex), "");
      LOG.error(
          "IoLink event does not look like expected. "
              + "No port information found for port {}.", portIndex);
      missingEventDataDetected = true;
    }
  }

  private void handleDataExtractionError(IAdapterRuntimeContext adapterRuntimeContext, int portIndex, Exception e) {
    adapterRuntimeContext
        .getLogger()
        .error(e);
    LOG.error("Data from IOLink sensor could not be extracted for port {}: {}", portIndex, e);
  }

  private void handleParseException(IAdapterRuntimeContext adapterRuntimeContext, ParseException e) {
    adapterRuntimeContext
        .getLogger()
        .error(e);
    LOG.error("IOLink master event could not be parsed.", e);
