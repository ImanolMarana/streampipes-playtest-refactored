...
public class InfluxDbStreamAdapter implements StreamPipesAdapter {
  ...
  public static class PollingThread implements Runnable {
    ...
    public void run() {
      ...
      private void runPollingLoop() throws SpRuntimeException {
        String lastTimestamp = getNewestTimestamp();
    
        while (!Thread.interrupted()) {
          try {
            Thread.sleep(pollingInterval);
            lastTimestamp = processQueryResult(lastTimestamp);
          } catch (InterruptedException e) {
            break;
          }
        }
      ...
      private String processQueryResult(String lastTimestamp) throws SpRuntimeException {
        ...
        if (queryResult.size() > 0) {
          ...
          collectEvents(queryResult);
        }
    
        return lastTimestamp;
      }
    
      private void collectEvents(List<List<Object>> queryResult) {
