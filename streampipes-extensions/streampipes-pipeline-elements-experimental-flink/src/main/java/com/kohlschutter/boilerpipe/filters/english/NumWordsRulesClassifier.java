...
public class NumWordsRulesClassifier implements BoilerpipeFilter {
  ...
  protected boolean classify(final TextBlock prev, final TextBlock curr, final TextBlock next) {
    if (curr.getLinkDensity() > 0.333333) {
      return curr.setIsContent(false);
    }

    if (prev.getLinkDensity() <= 0.555556) {
      return curr.setIsContent(classifyLowPrevLinkDensity(prev, curr, next));
    } else {
      return curr.setIsContent(classifyHighPrevLinkDensity(curr, next));
    }
  }

  private boolean classifyLowPrevLinkDensity(final TextBlock prev, final TextBlock curr, final TextBlock next) {
    if (curr.getNumWords() <= 16) {
      return curr.setIsContent(next.getNumWords() > 15 || prev.getNumWords() > 4);
    } else {
      return true;
    }
  }

  private boolean classifyHighPrevLinkDensity(final TextBlock curr, final TextBlock next) {
    if (curr.getNumWords() <= 40) {
      return curr.setIsContent(next.getNumWords() > 17);
    } else {
      return true;
    }
  }
