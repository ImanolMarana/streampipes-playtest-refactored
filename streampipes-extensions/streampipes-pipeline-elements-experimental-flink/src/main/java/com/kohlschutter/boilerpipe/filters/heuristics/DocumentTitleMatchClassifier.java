...
public final class DocumentTitleMatchClassifier implements BoilerpipeFilter {
  ...
  public DocumentTitleMatchClassifier(String title) {
    if (title == null) {
      ...
      this.potentialTitles = extractPotentialTitles(title);
    }
  }

  private Set<String> extractPotentialTitles(String title) {
    title = title.replace('\u00a0', ' ').replace("'", "").trim().toLowerCase();
    ...
    addLongestPart(potentialTitles, title, "[ ]*[\\|»|-][ ]*");
    addLongestPart(potentialTitles, title, "[ ]*[\\|»|:][ ]*");
    addLongestPart(potentialTitles, title, "[ ]*[\\|»|:\\(\\)][ ]*");
    addLongestPart(potentialTitles, title, "[ ]*[\\|»|:\\(\\)\\-][ ]*");
    addLongestPart(potentialTitles, title, "[ ]*[\\|»|,|:\\(\\)\\-][ ]*");
  ...
  private void addLongestPart(Set<String> potentialTitles, String title, String pattern) {
  ...
  private String getLongestPart(final String title, final String pattern) {
    ...
    return longestPart.isEmpty() ? null : longestPart.trim();
  }
