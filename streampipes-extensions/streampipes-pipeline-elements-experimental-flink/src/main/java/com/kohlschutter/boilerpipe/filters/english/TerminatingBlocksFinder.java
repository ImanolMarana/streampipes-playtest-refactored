...
public class TerminatingBlocksFinder implements BoilerpipeFilter {
  ...
  public boolean process(TextDocument doc) throws BoilerpipeProcessingException {
    ...
    for (TextBlock tb : doc.getTextBlocks()) {
      changes |= processTextBlock(tb);
    }
    
    return changes;
  }

  private boolean processTextBlock(TextBlock tb) {
    ...
    if (matchesEndIndicators(textLC)) {
      tb.addLabel(DefaultLabels.INDICATES_END_OF_TEXT);
      return true;
    }
    
    if (tb.getLinkDensity() == 1.0 && text.equals("Comment")) {
      tb.addLabel(DefaultLabels.INDICATES_END_OF_TEXT);
      return true;
    }
  ...
  private boolean matchesEndIndicators(String textLC) {
    return textLC.startsWith("comments")
        || startsWithNumber(textLC, textLC.length(), " comments", " users responded in")
