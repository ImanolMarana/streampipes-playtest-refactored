...
public final class ImageExtractor {
  ...
  private final class Implementation extends AbstractSAXParser implements ContentHandler {
    ...
    characterElementIdx++;
    if (inIgnorableElement == 0) {
      processCharacterBlock(ch, start, length);
    }
  }

  private void processCharacterBlock(char[] ch, int start, int length) {
    boolean highlight = contentBitSet.get(characterElementIdx);
    if (!highlight) {
      if (length == 0 || isJustWhitespace(ch, start, length)) {
        return;
      }
    }
  ...
  private boolean isJustWhitespace(char[] ch, int start, int length) {
