...
public class BoilerpipeHTMLContentHandler implements ContentHandler {
  ...
  public void flushBlock() {
    ...
    if (length == 1 && sbLastWasWhitespace) {
      resetBuffers();
      return;
    }

    final String[] tokens = UnicodeTokenizer.tokenize(tokenBuffer);
    TextBlock tb = createTextBlock(tokens);
    if (tb == null) {
      return;
    }
  ...
  private TextBlock createTextBlock(String[] tokens) {
    TokenCounts counts = countTokens(tokens);
    if (counts.numTokens == 0) {
      return null;
    }

    int numWordsInWrappedLines = calculateWrappedWords(counts);

    TextBlock tb = new TextBlock(textBuffer.toString().trim(), currentContainedTextElements,
        counts.numWords, counts.numLinkedWords, numWordsInWrappedLines, counts.numWrappedLines,
        offsetBlocks);
  ...
  private TokenCounts countTokens(String[] tokens) {
    TokenCounts counts = new TokenCounts();
    counts.currentLineLength = -1; // Don't count the first space
    ...
    for (String token : tokens) {
      if (ANCHOR_TEXT_START.equals(token)) {
        ...
        counts.incrementWordCounts();
        if (inAnchorText) {
          counts.numLinkedWords++;
        }
        counts.updateLineLength(token);
      } else {
        counts.numTokens++;
      }
    }
  ...
  private int calculateWrappedWords(TokenCounts counts) {
    if (counts.numWrappedLines == 0) {
      counts.numWrappedLines = 1;
      return counts.numWords;
    } else {
      return counts.numWords - counts.numWordsCurrentLine;
    }
  }
  ...
  private static class TokenCounts {
    ...
    void incrementWordCounts() {
      numTokens++;
      numWords++;
      numWordsCurrentLine++;
    }

    void updateLineLength(String token) {
